// Purpose of these functions is to clean the data from the API
...
export const jobReducer = (rawJob, parseRelatedJobs = true) => {
  ...
  // Recursion calling the jobReducer again to clean the jobs inside the relatedJobs field
  const relatedJobs = job.relatedJobs || [];
  if (!parseRelatedJobs) {
    job.relatedJobs = [];
  } else {
    job.relatedJobs = relatedJobs.data.map((relatedJob) =>
      jobReducer(relatedJob, false)
    );
  }
